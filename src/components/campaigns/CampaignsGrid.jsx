...
import { getAllCampaigns, getActiveCampaigns } from "@/data/campaigns";
import { 
  Sparkles,
  Clock,
  TrendingUp,
...
export default function CampaignsGrid() {
  ...
  const comingSoonCampaigns = allCampaigns.filter(c => c.status === "coming-soon");  const getFilteredCampaigns = () => {
    switch (filter) {
      case "active":
        return activeCampaigns;
      case "coming-soon":
        return comingSoonCampaigns;
      default:
        return allCampaigns;
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="text-center space-y-4">
        <div className="inline-flex items-center space-x-2 px-4 py-2 rounded-full bg-gradient-to-r from-primary/10 to-primary/5 border border-primary/20">
          ...
          <span className="text-sm font-semibold text-primary">FanSpark Campaigns</span>
        </div>
        ...
        <div>
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto leading-relaxed">
            Explore unique digital campaigns, each with their own rewards, games, and experiences. 
