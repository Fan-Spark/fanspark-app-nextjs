...
export default function CollectionSidebar({ 
  ...
  const handleNavigation = (item) => {
    ...
    const url = `/collections/${collection.slug}${item.path}`;
    router.push(url);
  };
  ...
  const isActiveItem = (item) => {
    const itemPath = `/collections/${collection.slug}${item.path}`;
    if (item.path === '/campaigns' && pathname === `/collections/${collection.slug}`) return true;
    ...
    <div className="flex flex-col h-full">
      ...
      {!isMobile && (
        <div className="mb-4 p-3 bg-gradient-to-r from-accent/10 to-accent/5 rounded-xl border border-border/20">
          <div className="flex items-center space-x-2 mb-2">
            ...
            <h2 className="text-sm font-semibold text-primary truncate">{collection.name}</h2>
          </div>
          ...
          <Badge variant="outline" className="text-xs bg-background/50 backdrop-blur-sm border-border/30 mt-2">
            {collection.navigation.length} Features
          </Badge>
        </div>
      )}
      ...
      <ScrollArea className="flex-1">
        <div className="space-y-1 p-1">
          {collection.navigation.map((item) => {
      ...
      {/* Back to Collections - Bottom Center */}
